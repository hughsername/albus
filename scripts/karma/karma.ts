...
import { deltaReputation, writeReputationToDynamocDB } from './countEntries';
...
const karma = async (app: AppCustomContext) => {
  ...
  app.message(/([\w<@>]+)\s*([\+\-]{2})/g, async ({ message, say }) => {
    if (message.subtype === undefined || message.subtype === 'bot_message') {
      if (message.text && typeof message.text === 'string' && message.text.trim() !== '' && message.user) {
        const changes = deltaReputation(message.text);
        const totals = await writeReputationToDynamocDB(changes);
