...
// Retrieve the top 5 usernames with the highest reputation using GSI
export const getBest5UsersByReputation = async (): Promise<KarmaRecord[]> => {
  try {
    const params: QueryCommand = new QueryCommand({
      ...
      ScanIndexForward: false, // Sort in descending order to get top 5 by reputation
      Limit: 5,
    });
    ...
    console.error("Error fetching top 5 users by reputation:", error);
    throw error;
  }
};
